...
function EquipmentCard({ item, eagerImage = false }: { item: Equipment; eagerImage?: boolean }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-lg border border-[#102018]/10 bg-white shadow-sm">
      <div className="relative h-64 overflow-hidden bg-[radial-gradient(circle_at_50%_35%,#ffffff_0%,#eef8dd_46%,#e6ece5_100%)]">
        <Link
          href={`/sprzet/${item.id}`}
          aria-label={`Zobacz szczegóły: ${item.brand} ${item.model}`}
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <Link
          href={`/sprzet/${item.id}`}
          className="mt-5 rounded-md border border-[#6f9f1f]/30 px-4 py-3 text-center text-sm font-black uppercase text-[#5f8818] transition-colors hover:border-[#6f9f1f] hover:bg-[#eef8dd]"
        >
          Zobacz szczegóły
        </Link>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-5 py-10 sm:px-6 lg:py-14">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item, index) => (
            <EquipmentCard key={item.id} item={item} eagerImage={index < 3} />
          ))}
        </div>
