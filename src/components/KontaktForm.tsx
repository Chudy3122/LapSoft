...
export function KontaktForm({ isLoggedIn }: KontaktFormProps) {
  ...
  const selectedExtraEquipmentItems = selectedEquipmentItems.filter(item => item.monthlyPrice)
  const selectedBaseEquipmentItems = selectedEquipmentItems.filter(item => !item.monthlyPrice)
  ...
  const buyoutMonthly = form.buyout && pkg
    ...
    : 0
  const total = basePrice + extraEquipmentTotal + addonsTotal + buyoutMonthly
            ...
            <label key={p.id} className="cursor-pointer">
              <input
                ...
                onChange={e => setForm(prev => ({
                  ...
                  equipmentIds: prev.equipmentIds.filter(id => equipment.find(item => item.id === id)?.monthlyPrice),
                }))}
                className="sr-only"
              />
        ...
        {(pkg || selectedEquipmentItems.length > 0) && (
          <div className="mt-6 rounded-lg border border-[#7DB122]/25 bg-[#eef8dd] p-4">
            ...
            {selectedExtraEquipmentItems.length > 0 && (
              <div className="mt-4">
                <p className="font-bold text-gray-500">Dodatkowe wyposażenie w racie</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {selectedExtraEquipmentItems.map(item => (
                    <span key={item.id} className="rounded-md bg-white px-3 py-1.5 text-xs font-black text-gray-700">
                      {item.brand} {item.model} +{item.monthlyPrice ?? 0} zł/mies.
                    </span>
                  ))}
                </div>
              </div>
      ...
      <section className="overflow-hidden rounded-lg bg-[#102018] text-white shadow-xl shadow-[#102018]/15">
        ...
        <div className="border-t border-white/10 p-6">
                  ...
                  {selectedEquipmentItems.map(item => (
                    <div key={item.id} className="flex justify-between gap-4 text-[#f4f9ed]/70">
                      ...
                      <span className="text-right font-black text-white">
                        {item.brand} {item.model}{item.monthlyPrice ? ` +${item.monthlyPrice} zł` : ''}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <div className="mb-4 rounded-lg border border-white/10 bg-white/[0.04] p-4 text-sm leading-6 text-[#f4f9ed]/65">
            {isLoggedIn
              ? 'Zapytanie zostanie zapisane w panelu klienta, bo formularz wysyłasz jako zalogowany użytkownik.'
